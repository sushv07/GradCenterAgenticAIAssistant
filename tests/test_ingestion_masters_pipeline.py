...
Index fixture is card-structured (P3 calibration). Card 3's program page is not
registered with the fetcher, exercising the graceful page-fetch-failure path.
...
# Note: the "ambiguous-studies" program page URL is deliberately NOT registered.
_PAGES = {
...
class TestPipeline(unittest.TestCase):
    def test_discovers_and_canonicalizes_three(self):
        with tempfile.TemporaryDirectory() as tmp:
            res = _run(Path(tmp))
            self.assertEqual(len(res.manifest.programs), 3)
            self.assertEqual(len([r for r in res.results if not r.skipped]), 3)

    def test_all_records_load_and_validate_without_errors(self):
        with tempfile.TemporaryDirectory() as tmp:
            res = _run(Path(tmp))
    ...
    def test_file_per_program_naming(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(files, ["ambiguous-studies.json", "applied-statistics.json",
                                     "data-science.json"])

    def test_page_fetch_failure_is_graceful_and_marks_unknown(self):
        with tempfile.TemporaryDirectory() as tmp:
            res = _run(Path(tmp))
            amb = next(r for r in res.results if r.program_id == "ambiguous-studies")
            self.assertTrue(any("program page fetch failed" in w for w in amb.warnings))
            program = load_program(Path(amb.canonical_path))
            # no program page consulted -> college/overview are 'unknown', not fabricated
            self.assertEqual(program.identity.college.data_status, DataStatus.UNKNOWN)
            self.assertEqual(program.overview.official_summary.data_status, DataStatus.UNKNOWN)
    ...
    def test_snapshots_written_and_hashed(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertGreaterEqual(len(snaps), 3)  # index + 2 fetchable program pages
    ...
    def test_does_not_write_to_production_corpus(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            _run(Path(tmp))
            if prod.exists():
                self.assertEqual(list(prod.glob("*.json")), [])
