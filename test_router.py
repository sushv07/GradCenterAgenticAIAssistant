...
Mocked callsites:
  router.find_advisor         — RapidFuzz in-memory (cheap but side-effectful)
  router._tool_detect_program — in-memory alias map
...
def _route(
    ...
    with (
        patch("router.find_advisor", return_value=advisor),
        patch("router._tool_detect_program", return_value=program),
